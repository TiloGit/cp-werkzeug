...
RUN dnf -y update && \
    ...
    dnf -y install podman fuse-overlayfs jq openssl wget zip unzip bind-utils findutils iputils nmap openldap-clients openssh openssh-clients which \
        --exclude container-selinux && \
    dnf clean all && \
    curl --no-progress-meter --location  https://mirror.openshift.com/pub/openshift-v4/clients/ocp/stable-4.9/openshift-client-linux.tar.gz |   \
